...
def get_EP_system(case_id: str) -> pic.EP_SYSTEM:
    case_meta = cdata.load_case_meta(case_id)
    return case_meta.get("ep_system")


def get_MAPPING_system(case_id: str) -> pic.MAPPING_SYSTEM:
    case_meta = cdata.load_case_meta(case_id)
    return case_meta.get("mapping_system")


def only_rf_events(case_id: str, event_df: pd.DataFrame) -> pd.DataFrame:
    if "notes" in event_df:  # tags
        return cdata.load_local_rfs(event_df=event_df)
    match get_EP_system(case_id):
...
def format_and_store_case_export(
    ...
    ep_system = get_EP_system(case_id)
    case_directory = case_directory or cdata.case_file_path(case_id, SIGNAL_SOURCE_BY_SYSTEM[ep_system])
    if "signals" in only:
        assert (
        ...
        case_meta = cdata.load_case_meta(case_id=case_id)
        channels = (
            case_meta["channels"]
    ...
    if "events" in only:
        ...
        if event_df is not None:
            csv_file = cdata.case_file_path(case_id, cdata.FileType.EPSYSTEM_EVENTS)
...
def load_events(case_id: str, outputs: list[str]) -> Union[pd.DataFrame, list[pd.DataFrame]]:
    ...
    Args:
        case_id (str): case_id
        outputs (list[str]): sequence of event dataframes to return. Subset of {'atrium_events', 'atrium_RFs', 'epsys_events', 'epsys_RFs', 'RFs'}
    ...
    if {"atrium_events", "atrium_RFs", "RFs"}.intersection(set(outputs)):
        results["atrium_events"] = cdata.load_local_events(case_id)
        if results["atrium_events"] is not None:
            results["atrium_RFs"] = cdata.load_local_rfs(event_df=results["atrium_events"])
    ...
    if {"epsys_events", "epsys_RFs"}.intersection(set(outputs)) or still_need_rfs:
        ep_log_file = cdata.case_file_path(case_id, cdata.FileType.EPSYSTEM_EVENTS)
        ...
        if not os.path.exists(ep_log_file):
            return None
        results["epsys_events"] = pd.read_csv(ep_log_file, parse_dates=["time"]).sort_values("time", ignore_index=True)
        if "epsys_RFs" in outputs or still_need_rfs:
            results["epsys_RFs"] = only_rf_events(case_id, results["epsys_events"])
            if still_need_rfs:
                results["RFs"] = results["epsys_RFs"]
...
def load_rf_events(case_id: str) -> pd.DataFrame:
    return load_events(case_id, outputs=["epsys_RFs"])


def load_case_signals(case_id: str) -> pd.HDFStore:
    signals_file = cdata.case_file_path(case_id, cdata.FileType.SIGNALS)
    ...
    return cdata.case_signals_db(case_id)


def generate_tags_from_ensite(case_id: str, parse_types: list[str] = ["geometry", "RF", "MAP"]):
    return import_ensite_export(case_id, parse_types, lambda cid: load_events(cid, ["epsys_RFs"]))


def infer_map_system_offset(case_id: str) -> float:
    match get_MAPPING_system(case_id):
        case pic.MAPPING_SYSTEM.ENSITE:
            return ensc.infer_ensite_offset(case_id, lambda cid: load_events(cid, ["epsys_RFs"]))


def generate_tags_from_carto(case_id: str, parse_types: list[str] = ["geometry", "RF", "MAP"], opts: dict = {}):
    return crto.import_carto_export(case_id, parse_types, lambda cid: load_events(cid, ["epsys_RFs"]), opts)
...
def _update_meta_channels(case_id: str, raw_channel_names: list[str]) -> None:
    ...
    cdata.update_case_meta(case_id, ["channels"], meta_channels[::-1], append=False)


def plot_raw_data_coverage(case_id: str):
    ep_system = get_EP_system(case_id)
    case_directory = cdata.case_file_path(case_id, SIGNAL_SOURCE_BY_SYSTEM[ep_system])
